export default async function loginPage(ctx) {
  ...
  function createPetals(layer) {
    ...
    // إعادة تشغيل تلقائي للبتلات كل 18 ثانية
    const petalInterval = setInterval(() => {
      if (layer.isConnected) createPetals(layer);
      else clearInterval(petalInterval);
    }, 18000);
    cleanupFns.push(() => clearInterval(petalInterval));
  }
