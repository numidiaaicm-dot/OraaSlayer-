...
export default async function chatPage(ctx) {
    ...
    <section class="chat-page" id="chatPage">
      ...
      <aside class="nav-sidebar" id="navSidebar">
        ...
        <nav>
          ...
          <a href="/event_gacha/spin" data-link class="nav-sidebar-link"><i data-lucide="gift"></i> الهدايا</a>
        </nav>
      ...
      <nav class="main-nav mobile-only">
        ...
        <a href="/event_gacha/spin" data-link class="nav-item"><div class="nav-icon-wrap"><i data-lucide="gift"></i></div><span class="nav-label">هدايا</span></a>
  ...
  function renderUsersList() {
    ...
    usrList.innerHTML = sorted.map((u) => `
      <div class="usr-item" onclick="viewProfile('${u.uid}')">
        <img src="anime_img/${u.avatar || "01.jpg"}" class="usr-av" loading="lazy" onerror="this.src='anime_img/01.jpg'">
        <div class="usr-info">
  ...
  function createMsgEl(id, data, isPend = false, grouped = false) {
    ...
    let badge = "";
    if (s.verifyIcon) badge = `<img src="vrf/${s.verifyIcon}" class="vrf-badge" onerror="this.style.display='none'">`;
    else if (s.role === "Admin Root" || isAI) badge = `<img src="vrf/admin_verify.png" class="vrf-badge" onerror="this.style.display='none'">`;

    const frame = s.currentFrame ? `<img src="frame_data/${s.currentFrame}" class="frame-img" onerror="this.style.display='none'">` : "";
    ...
    const delBtn = canDel ? `<button class="del-btn" onclick="deleteMsg('${id}')" title="حذف"><i class="fa-solid fa-trash"></i></button>` : "";
    ...
    div.innerHTML = `
      <div class="av-wrapper" onclick="${isPend ? "" : `viewProfile('${data.senderId}')`}">
        <img src="anime_img/${s.avatar || "01.jpg"}" class="av-img" loading="lazy" onerror="this.src='anime_img/01.jpg'">
        ${frame}
        ${badge}
      </div>
  ...
  function handleMentions(text) {
    ...
    mentionSuggest.innerHTML = filt.map((u) => `
      <div class="mention-item" onclick="insertMention('${esc(u.name || "").replace(/'/g, "\\'")}', '${u.uid}')">
        <img src="anime_img/${u.avatar || "01.jpg"}" onerror="this.src='anime_img/01.jpg'">
  ...
  async function fetchAI(text) {
    try {
      ...
      console.warn("AI unreachable:", e.message);
